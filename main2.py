...
# Combined function to import a file, split it into objects, slice, and export G-code
# Combined function to import a file, split it into objects, slice, and export G-code
def import_split_slice_export(directory_path, export_path, folder_name, file_index):
    try:
        pattern = re.compile(r"^stl_\d+\.stl$")
        directory_path = "F:\\LightSwitchWallPlates\\Images\\Products\\Gold-Sliced2\\3DModels\\Main"
        ...
        for filename in os.listdir(directory_path):
            if pattern.match(filename):  # Check if the filename matches the pattern
                ...
                # Use import_file as the file_path for importing
